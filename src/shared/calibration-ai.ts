...
export const understandingCalibrationSystemPrompt = `너는 학생이 비문학 지문을 이해하도록 돕는 한국어 독해 보조 AI이다. 학생은 아직 이후 활동이 무엇인지 모른다. 학생의 질문에 자연스럽게 답하라. 지문과 보조자료를 우선 사용하되, 필요한 경우 일반적인 배경 설명을 쉽게 덧붙일 수 있다. 학생이 쉬운 설명, 예시, 비유, 요약, 글 형태의 정리를 요청하면 제공할 수 있다. 다만 이후 활동을 언급하지 말고, 학생에게 평가가 있을 것처럼 암시하지 마라. 과도하게 칭찬하거나 ‘잘 이해했다’고 판단해주지 마라. 기본 답변은 3~6문장으로 짧게 하되, 학생이 길게 요청하면 더 자세히 답하라. 불확실한 내용은 불확실하다고 말하라. 주제와 무관한 대화는 짧게 답하고 다시 글의 내용으로 돌아오게 하라.`;
...
export const assistantReplyForCalibration = (input: {
  ...
  if (includesTag(tags, "generated_explanation_request") || includesTag(tags, "summary_request")) {
    return `${input.topic}은 지문에서 말하는 핵심 내용을 중심으로 정리하면 좋아요. ${passageHint} 이 부분을 보면, 무엇이 중요한지와 왜 조심해야 하는지를 함께 볼 수 있어요.${context}`;
  }
  if (includesTag(tags, "easy_explanation_request") || includesTag(tags, "definition_request") || includesTag(tags, "clarification_request")) {
    return `${input.topic}을 쉬운 말로 보면, 지문에서 설명한 현상이 왜 생기고 어떤 영향을 주는지 이해하는 과정이에요. 먼저 "${passageHint}"라는 문장이 무엇을 말하는지부터 차근차근 보면 좋아요.${context}`;
  }
  if (includesTag(tags, "example_request") || includesTag(tags, "analogy_request")) {
    return `예를 들면 ${input.topic}은 일상에서 비슷한 상황과 연결해 생각할 수 있어요. 다만 비유는 이해를 돕는 도구라서, 지문 속 사실과 완전히 같다고 보면 안 돼요.${context}`;
  }
  if (includesTag(tags, "why_how_request")) {
    return `${input.topic}의 이유나 원리를 볼 때는 '무엇이 달라지는가'와 '그래서 어떤 결과가 생기는가'를 나누면 이해하기 쉬워요. 지문의 한 문장을 골라 그 연결을 직접 설명해 보세요.${context}`;
  }
  if (includesTag(tags, "limitation_request") || includesTag(tags, "verification_request")) {
    return `${input.topic}을 이해할 때는 맞는 경우와 조심해야 할 경우를 함께 봐야 해요. 지문에서 제한, 우려, 예외를 나타내는 표현을 찾아보면 그 문장이 항상 맞는지 판단하는 데 도움이 됩니다.${context}`;
  }
  return `${input.topic}에 대해 더 확인하려면 질문을 조금 더 좁혀 보세요. 뜻, 이유, 예시, 요약, 한계 중 하나를 골라 물으면 그 부분을 중심으로 설명할 수 있어요.${context}`;
};
