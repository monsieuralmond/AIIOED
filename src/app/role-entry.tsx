...
export function RoleEntry(props: RoleEntryProps): ReactElement {
  ...
  useEffect(() => {
    setSelectedRole(props.mode === "entry" ? null : props.mode);
  }, [props.mode]);

  const submitStudentCredentials = async (): Promise<void> => {
    setStudentError("");
    const hasParticipantCode = studentParticipantCode.trim().length > 0;
    const hasStudentCredentials = studentLoginId.trim().length > 0 && studentPassword.trim().length > 0;
    if (!hasParticipantCode && !hasStudentCredentials) {
      setStudentError("참여자 코드 또는 아이디와 비밀번호를 입력하세요.");
      return;
    }
    ...
    try {
      if (!(await props.onStudentCredentials({ loginId: studentLoginId, participantCode: studentParticipantCode, password: studentPassword }))) {
        setStudentError("참여자 코드 또는 학생 계정 정보가 맞지 않습니다");
      }
    } finally {
  ...
  const submitTeacher = async (): Promise<void> => {
    ...
    try {
      if (!(await props.onTeacher(teacherLoginId, teacherPassword))) setTeacherError("아이디 또는 비밀번호가 맞지 않습니다");
    } finally {
  ...
  const submitAdmin = async (): Promise<void> => {
    ...
    try {
      if (!(await props.onAdmin(adminLoginId, adminPassword))) setAdminError("아이디 또는 비밀번호가 맞지 않습니다");
    } finally {
  ...
  const submitStudentForm = (event: FormEvent<HTMLFormElement>): void => {
    event.preventDefault();
    void submitStudentCredentials();
  };

  const submitTeacherForm = (event: FormEvent<HTMLFormElement>): void => {
    event.preventDefault();
    void submitTeacher();
  };

  const submitAdminForm = (event: FormEvent<HTMLFormElement>): void => {
    event.preventDefault();
    void submitAdmin();
  };
  ...
  const description =
    selectedRole === null
      ...
      : selectedRole === "student"
        ? "참여자 코드만 입력하거나, 아이디와 비밀번호로 들어갈 수 있습니다."
        ...
        <Surface className="role-entry">
          ...
          {selectedRole === "student" ? (
            <form className="login-section" aria-label="학생 로그인" onKeyDown={(event) => submitLoginOnEnter(event, submitStudentCredentials, studentPending)} onSubmit={submitStudentForm}>
              ...
              <Button disabled={studentPending} type="submit" variant="primary">{studentPending ? "확인 중" : "로그인"}</Button>
            </form>
          ) : null}
          ...
          {selectedRole === "teacher" ? (
            <form className="login-section" aria-label="교사 로그인" onKeyDown={(event) => submitLoginOnEnter(event, submitTeacher, teacherPending)} onSubmit={submitTeacherForm}>
              ...
              <Button disabled={teacherPending} type="submit" variant="primary">{teacherPending ? "확인 중" : "로그인"}</Button>
            </form>
          ) : null}
          ...
          {selectedRole === "admin" ? (
            <form className="login-section" aria-label="관리자 로그인" onKeyDown={(event) => submitLoginOnEnter(event, submitAdmin, adminPending)} onSubmit={submitAdminForm}>
              ...
              <Button disabled={adminPending} type="submit" variant="primary">{adminPending ? "확인 중" : "로그인"}</Button>
            </form>
          ) : null}
        </Surface>
      </div>
