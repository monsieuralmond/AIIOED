...
import type { PilotSession, PilotState, StudentAccount, StudentWorkStatus, TeacherReviewStatus, TeacherReviewUpdate } from "../shared/types";
...
export function TeacherReview(props: { readonly state: PilotState; readonly onBack: () => void; readonly onUpdateReview: (sessionId: string, input: TeacherReviewUpdate) => void }): ReactElement {
  ...
  const studentRows = props.state.students.map((student): StudentProcessRow => {
    const session = sessionForStudent(props.state, student.id, props.state.activeAssignmentId);
    return { session, status: sessionStatus(props.state, student.id, props.state.activeAssignmentId), student };
  });
  ...
  const selectedSession = selectedStudentId.length === 0 ? null : sessionForStudent(props.state, selectedStudentId, props.state.activeAssignmentId);
  return (
    <main className="teacher-review-page">
      <section className="teacher-review-header">
        <div>
          <h1>학생 현황</h1>
          <p>학생별 진행 상태와 대화, 초안, 붙여넣기, 최종 제출 기록을 확인합니다.</p>
        </div>
      ...
      <section className="teacher-review-layout">
        <Surface className="student-status-list">
          <header className="student-status-heading">
            <div>
              <h2>파일럿 반</h2>
              <p>{studentRows.length}명 중 {statusCounts.submitted}명 제출</p>
            </div>
          </header>
          ...
          {visibleRows.map((row) => {
            const rowClassName = row.student.id === selectedStudentId ? "student-status-row selected" : "student-status-row";
            return (
