alter table public.assignments
  drop constraint if exists assignments_class_group_id_fkey;

alter table public.assignments
  add constraint assignments_class_group_id_fkey
  foreign key (class_group_id) references public.classes(id) on delete set null;
...
as $$
  select jsonb_build_object(
    'version', '016_preserve_assignments_when_deleting_classes',
    'assignments_class_group_delete_sets_null', exists (
      select 1
      from information_schema.referential_constraints
      where constraint_schema = 'public'
        and constraint_name = 'assignments_class_group_id_fkey'
