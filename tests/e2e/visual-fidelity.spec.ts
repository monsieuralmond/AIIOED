...
test("desktop and mobile layout respect clone constraints", async ({ page }) => {
  ...
  expect(workspace?.x).toBeGreaterThanOrEqual(180);
  expect(workspace?.width).toBeGreaterThan(1300);
  expect(workPane?.x).toBeGreaterThanOrEqual(180);
  expect(workPane?.width).toBeGreaterThan(700);
  ...
  expect(mobileWorkspace?.width).toBeGreaterThan(340);
  expect(mobileWorkPane?.x).toBeGreaterThanOrEqual(0);
