...
test("submitted writing coach task can be reopened from the student assignment list", async ({ page }) => {
  await enterStudent(page);
  await page.getByRole("button", { name: "이해했어요" }).click();
  await page.getByLabel("중심 생각").fill("일회용 플라스틱은 줄여야 한다");
  await page.getByLabel("근거 또는 예시 1").fill("분해가 오래 걸린다");
  ...
  const reopenButton = page.getByRole("button", { name: "다시 열기" });
  await expect(reopenButton).toBeEnabled();
  await reopenButton.click();
  await expect(page.getByText("제출 완료")).toBeVisible();
  await expect(page.getByTestId("draft-editor")).toHaveValue("일회용 플라스틱은 줄여야 한다. 분해가 오래 걸리고 생태계에 피해를 주기 때문이다.");
});
