...
import { enterStudent, enterTeacher, openTeacherExport } from "./helpers";
...
test("export view exposes research JSON shape", async ({ page }) => {
  ...
  expect(exported).toHaveProperty("students.0.id", "student-minseo");
  expect(exported).toHaveProperty("students.0.studentNumber", 1);
  expect(exported).toHaveProperty("students.0.loginId", "minseo");
  expect(exported).toHaveProperty("students.0.participantCode", "S-MINSEO");
...
test("export captures revision check attempts for later labeling", async ({ page }) => {
  ...
  expect(JSON.stringify(exported)).toContain("개요에 쓴 근거 두 가지가 초안에 아직 모두 들어가지 않았어요.");
